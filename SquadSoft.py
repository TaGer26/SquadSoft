...
from calculate_elevation import calculate_elevation
...
class SquadSoft():
    def __init__(self, window):
        self.window = window
        self.main_canvas = CTkCanvas(self.window, width=main.winfo_screenwidth(), height=main.winfo_screenheight(),
                                     bg='#242424')
    ...
    def menu(self):
        ...
        # menu_content
        soft_nameL = CTkLabel(menuF, text=f'SquadSoft v{self.version} MODDED', font=('Arial Black', 12)).place(x=5, y=0)
        ...
        def change_frequency(value):
            zoom_frequency_label.configure(text=f'{int(value)} ms')
            self.menu_zoom_frequency = int(value)

        zoom_frequency = CTkSlider(menuF, from_=1, to=16, number_of_steps=8, command=change_frequency)
        zoom_frequency.place(x=215, y=175)
        zoom_frequency.set(1)

        zoom_frequency_label_t = CTkLabel(menuF, text=f'Время обновления', font=('Arial Black', 12))
        zoom_frequency_label_t.place(x=35, y=170)

        zoom_frequency_label = CTkLabel(menuF, text=f'1 ms', font=('Arial Black', 10))
        zoom_frequency_label.place(x=430, y=168)
        ...
        def apply_settings():
            ...
            self.zoom_f = self.menu_zoom_frequency
    ...
    def gp_table(self):
        ...
        self.gp_weapons = {1: {'name': 'Австралия SL40 ', 'path': r'GP\sl40.png', 'size': (625, 330)},
                           ...
                           10: {'name': 'Турция AK40GL (ВСЁ летит левее на 2-3 градуса)', 'path': r'GP\ak40gl.png',
                                'size': (625, 250)},
        ...
        self.gp_weapon = 5
        # gp_table_widget
        gp_tableL = CTkLabel(self.window, image=CTkImage(dark_image=Image.open(self.gp_weapons[self.gp_weapon]['path']),
                                                         size=(625 * float(self.settings['gp_scale']),
                                                               330 * float(self.settings['gp_scale']))), text='')
        gp_nameL = CTkLabel(self.window, text=self.gp_weapons[self.gp_weapon]['name'], font=('Arial Black', 15),
                            text_color=self.settings['text_color'], fg_color='#242424')
        # move_table
        ...
        def move_table(e):
            ...
            gp_nameL.place(x=current_x, y=current_y + (
                        self.gp_weapons[self.gp_weapon]['size'][1] * float(self.settings['gp_scale'])) + 10)
        ...
        def gp_weapon_switchF():
            if self.gp_state:
                if self.gp_weapon < 12:
                    ...
                    self.gp_weapon = 1
                gp_tableL.configure(image=CTkImage(dark_image=Image.open(self.gp_weapons[self.gp_weapon]['path']),
                                                   size=((self.gp_weapons[self.gp_weapon]['size'][0] * float(
                                                       self.settings['gp_scale'])), (
                                                                     self.gp_weapons[self.gp_weapon]['size'][1] * float(
                                                                 self.settings['gp_scale'])))))
    ...
    def calculate_distance(self):
                    ...
                    if self.click_count == 2:
                        ...
                        if mode == 'real':
                            if self.pix_distance != 0:
                                ...
                                elevation = calculate_elevation(real_dist)
                                azimuth = calculate_azimuth(x1,y1,x2,y2)
                                def show_distance():
                                    line = self.main_canvas.create_line(x1, y1, x2, y2, width=4,
                                                                        fill=self.settings['text_color'])
                                    distanceL = CTkLabel(self.window, text=f'{real_dist} м\n{elevation} m i l\n{azimuth} °', font=('Arial Black', 15),
                                                         text_color=self.settings['text_color'], fg_color='#242424')
                                    distanceL.place(x=x2, y=y2 + 5)

                                    distanceL2 = CTkLabel(self.window,
                                                         text=f'{real_dist} м {elevation} mil {azimuth} °',
                                                         font=('Arial Black', 13),
                                                         text_color='#EEEEEE', fg_color='#242424')
                                    distanceL2.place(x=890, y=1060)
        ...
        def switch_distance():
            if self.calculate_state:
                ...
                def show_distance():
                    distanceL = CTkLabel(self.window, text=f'{self.pnr_distance}м', font=('Arial Black', 25),
                                         text_color=self.settings['text_color'], fg_color='#242424')
        ...
        def zoomF():
            ...
            def update_image():
                if self.zoom_flag:
                    ...
                    self.window.after(self.zoom_f, update_image)
            update_image()
        def zoom_switch():
            if self.zoom_state:
                if self.zoom_flag:
